...
// ── Notification Sender Component (appended to dashboard)
export function NotifSender() {
  const [form, setForm] = useState({ title: '', body: '', type: 'news', url: '/', price: '' })
  ...
  const send = async () => {
    ...
    const res = await fetch('/api/push/send', {
      ...
      body: JSON.stringify({ target: 'all', ...form }),
    })
    const data = await res.json()
    setSending(false)
    setResult(res.ok ? `✅ Inviata a ${data.sent} dispositivi` : `❌ ${data.error}`)
  }

  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: 20, boxShadow: '0 2px 0 #d8d2ca, 0 6px 20px rgba(0,0,0,.06)', marginTop: 20 }}>
      <div style={{ fontWeight: 800, fontSize: 14, color: '#1a2e1a', marginBottom: 16 }}>📣 Invia notifica push a tutti gli utenti</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#888', display: 'block', marginBottom: 4 }}>Tipo</label>
          <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}
            style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #e0dbd2', borderRadius: 10, fontSize: 13, fontWeight: 600, background: '#f5f1eb' }}>
            <option value="news">📰 News</option>
            <option value="offer">💰 Offerta</option>
            <option value="maint">🔧 Manutenzione</option>
            <option value="generic">🏍️ Generico</option>
          </select>
        </div>
        <div>
          <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#888', display: 'block', marginBottom: 4 }}>URL (click notifica)</label>
          <input value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))}
            style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #e0dbd2', borderRadius: 10, fontSize: 13, background: '#f5f1eb' }} />
        </div>
      </div>
      {['title', 'body', 'price'].map(k => (
        <div key={k} style={{ marginBottom: 10 }}>
          <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#888', display: 'block', marginBottom: 4 }}>
            {k === 'title' ? 'Titolo' : k === 'body' ? 'Testo' : 'Prezzo (opzionale, solo offerte)'}
          </label>
          <input value={(form as any)[k]} onChange={e => setForm(f => ({ ...f, [k]: e.target.value }))}
            placeholder={k === 'title' ? 'Es. Nuove offerte per la tua moto!' : k === 'body' ? 'Es. Olio Motul 10W40 −20% solo oggi' : 'Es. €22.90'}
            style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #e0dbd2', borderRadius: 10, fontSize: 13, background: '#f5f1eb' }} />
        </div>
      ))}
      {result && <div style={{ padding: '10px 12px', borderRadius: 10, background: result.startsWith('✅') ? 'rgba(42,122,64,.08)' : 'rgba(200,50,50,.08)', fontSize: 13, fontWeight: 600, color: result.startsWith('✅') ? '#1a7a40' : '#b03020', marginBottom: 10 }}>{result}</div>}
      <button onClick={send} disabled={sending || !form.title || !form.body}
        style={{ padding: '12px 24px', background: '#2a6644', color: '#fff', border: 'none', borderRadius: 100, cursor: 'pointer', fontWeight: 700, fontSize: 13, opacity: sending ? .6 : 1 }}>
